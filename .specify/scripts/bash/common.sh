...
check_feature_branch() {
    ...
    if [[ "$has_git_repo" != "true" ]]; then
        echo "[specify] 경고: Git 저장소가 감지되지 않았습니다. 브랜치 유효성 검사를 건너뜁니다." >&2
        return 0
    fi
    ...
    if [[ "$is_sequential" != "true" ]] && [[ ! "$branch" =~ ^[0-9]{8}-[0-9]{6}- ]]; then
        echo "오류: 피처 브랜치가 아닙니다. 현재 브랜치: $raw" >&2
        echo "피처 브랜치 이름은 다음과 같이 지정해야 합니다: 001-feature-name, 1234-feature-name, 또는 20260319-143022-feature-name" >&2
        return 1
    fi
...
find_feature_dir_by_prefix() {
    ...
    if [[ ${#matches[@]} -eq 0 ]]; then
        ...
        # 여러 개가 매칭되는 경우 - 정상적인 이름 규칙에서는 발생하지 않아야 함
        echo "오류: 접두사 '$prefix'를 사용하는 여러 스펙 디렉토리가 발견되었습니다: ${matches[*]}" >&2
        echo "접두사당 하나의 스펙 디렉토리만 존재하도록 구성하십시오." >&2
        return 1
    fi
}
...
" 2>"$py_stderr")
                            ...
                            if [ "$yaml_warned" = false ] && grep -q 'yaml_missing' "$py_stderr" 2>/dev/null; then
                                echo "경고: PyYAML을 사용할 수 없습니다. 작성(composition) 전략이 무시될 수 있습니다." >&2
                                yaml_warned=true
                            fi
                            rm -f "$py_stderr"
                        fi
        ...
        case "$strat" in
            ...
            wrap)
                case "$layer_content" in
                    *'{CORE_TEMPLATE}'*) ;;
                    *) echo "오류: wrap 전략에 {CORE_TEMPLATE} 플레이스홀더가 누락되었습니다." >&2; return 1 ;;
                esac
                ...
                ;;
            *) echo "오류: 알 수 없는 전략입니다: '$strat'" >&2; return 1 ;;
        esac
    done
