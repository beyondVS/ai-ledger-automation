<!--
[Sync Impact Report]
- Version Change: v1.25.0 -> v1.26.0
- Ratified: 2026-05-29 | Last Amended: 2026-06-23
- Key Principles Defined:
  ...
  3. III. 3단계 하이브리드 영수증 파싱 전략 및 비용 최적화 (3-Tier Hybrid Pipeline & Cost Optimization)
  ...
  10. X. 실 인프라 고가용성 격리 및 TDD 기반 포트 보안 수호 (Production Isolation & Port Security)
- Added/Modified: (1) google-generativeai 라이브러리를 제거하고 google-genai 최신 공식 SDK 및 litellm 패키지를 도입하여 로컬 Ollama(qwen2.5:14b-instruct-q4_K_M) 연동 및 다이내믹 라우팅 분기 체계를 구축. (2) PDF 파싱 시 기계적 텍스트 파싱을 배제하고, PDF 원본 바이트를 Gemini 멀티모달 API에 application/pdf 파트를 통해 네이티브하게 멀티모달 분석을 태움(v1.7.0). (3) LiteLLM Router(litellm.Router)를 도입하여 로컬 환경에서는 Ollama qwen2.5:14b-instruct-q4_K_M를 최우선 주 모델 및 폴백 모델로 가동하고, 프로덕션(DEBUG=False) 환경에서만 Gemini-2.5-Flash를 우선적으로 라우팅하는 다이내믹 라우터(ReceiptLLMClient) 체계로 전격 통합(v1.8.0). (4) 백엔드, Celery, 프론트엔드 전체 Dockerizing 완료 및 핫 리로딩 인프라 통합 구축(v1.9.0). (5) 동일 상품 연속 결제 오탐지 방지를 위해 approval_number 필드를 신설하고, 승인번호 고유성 대조 및 1분(60초) 임계 시각 대조 하이브리드 중복 방어 알고리즘을 도입. 프론트엔드 수정 내역 모달(FE-05-B) 내 유실되거나 유효하지 않은 카테고리 데이터 바인딩 유입 시 '미분류'로 자동 폴백하는 안전 바인딩 정책을 적용하여 누수를 제거함(v1.10.0). (6) 이메일 포워딩 수집 및 SPF/DKIM 보안 필터링의 구현 일정을 4주 개발 로드맵에서 제외하고 차후 확장 계획 백로그로 안전하게 이관 보존함(v1.10.1). (7) 동기식 레거시 파서(CostControlParser) 및 관련 태스크(process_llm_fallback_task)를 영구 제거하고 Celery 기반 비동기 파이프라인으로 일원화. 중복 결제 발생 시 재전파(re-raise) 처리 대신 안전한 FAILED 실패 상태 반환 구조로 전환(v1.11.0). (8) 백엔드 컨테이너의 파이썬 실행 환경을 3.13-slim으로 업그레이드하고, pyproject.toml 설정 파일을 프로젝트 루트에 단일화(Single Source of Truth)하여 Ruff 린트/포맷 룰을 워크스페이스 전역으로 일치시킴(v1.11.0). (9) Gemini API 호출 장애 시 로컬 Ollama 폴백 가동 대역에서 base64 데이터 디코딩 규격 충돌(prefix 유무) 문제를 방지하기 위해, parse_receipt 메서드 단에서 try-except 블록으로 에러를 캐치하고 Ollama에 맞는 무접두사 base64 스트링으로 페이로드를 동적 가공 재인서트하는 방어벽을 구축함(v1.11.0). (10) 사용자의 고유 타임존 설정을 보존하기 위해 User 모델에 timezone 필드를 추가하고, 영수증 결제일시 파싱 시 외부 의존성(dateparser) 추가 없이 파이썬 내장 표준 라이브러리(datetime, zoneinfo) 및 User.timezone을 기반으로 로컬 시간대를 UTC 기준 Timezone-aware 시간대로 정밀 정규화하는 파이프라인 및 중복 검증 태스크를 구축함. LLM API 스키마/프롬프트를 개정하여 Naive 일시만 추출하도록 통제함(v1.13.0). (11) ReceiptParser(BypassParser/ReceiptLLMClient)의 파싱 결과 및 create_ledger_transactional 적재 서비스 간의 계약(Contract) 단일화를 위해 기존 dict 구조를 Pydantic DTO(ReceiptSchema) 체계로 전면 전환 및 일원화하여 정적 타입 안전성을 확보함. 레거시 dict 분할 입력 포맷 및 FailedTask payload 조회를 수행하는 레거시 테스트에 대한 하위 호환성을 100% 보장하도록 복합 변환 및 폴백 템플릿 필터링 체계를 보강함(v1.14.0). (12) 가계부 수동 수정 화면에서 결제일자 입력 폼을 datetime-local 타입으로 통일하고 타임존 정규화 결함을 해소함(v1.15.0). (13) JWT Refresh Token을 httpOnly 쿠키로 주입하고, Access Token만 sessionStorage에 보관하며, 라우터 가드에서 JWT Payload를 Base64 파싱하여 is_staff 권한을 위변조 검증하는 httpOnly 이중 토큰 인증 아키텍처를 구축. 프론트엔드 대시보드 화면에 데스크톱 브레이크포인트를 적용한 반응형 2열 그리드 적응형 레이아웃을 도입하여 UX/UI를 강화함(v1.16.0). (14) 사용자별 월별 예산 관리 모델(MonthlyBudget) 및 UNIQUE (user, budget_month) 제약 조건을 도입. 당월 카테고리 소비 분포(원형 차트), N개월간의 지출 추이(막대 차트), 소진 속도에 반응하는 예산 게이지 및 TOP 3 지출 가맹점 랭킹 요약을 단일 DTO로 초고속 반환하는 /api/ledgers/dashboard/ API 및 실시간 모달 예산 수정을 지원하는 /api/budgets/ API와 프론트엔드 리액티브 바인딩을 완료하여 가계부 UI 고도화 1단계를 달성함(v1.17.0). (15) 정적 정규식 매핑 기반의 가맹점 템플릿 바이패스(Bypass) 캐싱 아키텍처를 전면 제거/비활성화하고, 로컬의 경제성과 상용 LLM의 정밀성을 융합한 "3단계 하이브리드 영수증 파싱 전략(Local OCR + Ollama qwen2.5:14b-instruct-q4_K_M ➔ Gemini-2.5-Flash Text-only ➔ Gemini-2.5-Flash Vision)"을 구축하여 파싱 신뢰성 99% 달성 및 입력 이미지 토큰 비용을 95% 이상 최적화함(v1.18.0). (16) 사용자가 고유 타임존을 설정 및 변경할 수 있도록 PATCH /api/v1/accounts/timezone/ API와 드롭다운 UI 환경설정 탭을 구현하고, 사용자 로컬 일자별로 지출액/건수를 집계해 보여주는 grid 기반 월별 캘린더 뷰와 복합 다차원 필터링(상호, 카테고리, 기간, 금액) 패널 및 E2E 실시간 리액티브 연동을 구축하여 가계부 UI 고도화 2단계를 완료함(v1.19.0). (17) 템플릿 비활성화에 따라 DTO 스키마(ReceiptSchema) 내의 proposed_date_pattern 및 proposed_amount_pattern 필드와 프롬프트 조립에 사용되던 LOCAL_TEXT_PROMPT_EXTENSION 상수를 완전 영구 삭제 처리함(v1.19.1). (18) 3주차 비동기 아키텍처 튜닝을 완수하여 api_server 커넥션 풀 최대 5개, Celery 3개 합산 8개 이하 제약을 엄격히 제어함. 50종 일시 유입 부하 테스트 환경에서 트랜잭션 원자성과 중복 차단 100% 무결성을 증명하였으며, 비동기 처리 성능 및 3-Tier 하이브리드 파이프라인 성공율 통계를 자동 표출하는 ReceiptLoadTestReporter와 크로스 플랫폼 대칭형 E2E 실행 스크립트(run_load_test.ps1/sh)를 구축하여 검증을 완료함(v1.20.0). (19) 모바일 PWA 환경에서 beforeinstallprompt 이벤트를 가로채고 3초 지연 후 커스텀 하단 배너를 제공하며, iOS Safari 환경을 모의 감지하여 하단 공유 메뉴 조준 말풍선 툴팁 가이드(공유 아이콘 문양 SVG 포함)를 노출하고, 닫기 시 7일 쿨다운 및 Standalone 모드 진입 시 자동 미노출 차단을 수립하는 3-Tier PWA 모바일 설치 가이드 UI를 구축함(v1.21.0). (20) 로컬 개발 및 실행 환경 편의성을 극대화하기 위해 Supabase Free plan 및 AWS Free tier 등의 DB 인프라 제한에 종속되었던 RDBMS 커넥션 풀 합산 8개 이하 강제 제약을 완전 삭제하고, 개별 워커 및 API 서버의 가동 부하에 맞추어 유연하게 커넥션 풀을 할당하도록 규정을 개정함(v1.22.0). (21) 오프라인 상태 단말 복귀 시 지연 도달한 알림의 IndexedDB 로컬 캐싱(CachedNotification)을 구축하고, 중복 캐싱 방지를 위한 멱등성(Upsert) 분기 및 단말의 백그라운드 푸시 수신 시 백엔드 상태를 DELIVERED로 즉각 갱신하는 Acknowledgment POST API와 포그라운드 진입 시의 자동 델타 동기화(Sync GET API)를 구현함. 또한, 로컬 캐시의 30일 초과 데이터 및 100개 상한을 관리하는 가비지 컬렉션(GC)을 안전한 독립 트랜잭션 흐름으로 구축하고, E2E 검증용 백엔드 시딩을 테스트 단에서 직접 쉘 기동하도록 통제하여 프로덕션 뷰 오염을 전면 차단함(v1.23.0). (22) 대규모 실 인프라 환경 고가용성을 위한 CPU/Memory limits 고정 기술, 외부 포트 차단 및 Nginx 프록시 인그레스 단일화(prod-bridge망 격리), Named Volume(postgres_data) 이식성 수호 및 pytest 기반 TDD 인프라 검증 파이프라인 통과(v1.24.0). (23) VAPID 웹푸시 알림망에 대해 정규 Celery 비동기 큐를 우회하여 특정 사용자에게 동기로 즉시 테스트 알림을 발송하는 custom django-admin 커맨드(trigger_test_push) 구현 및 이를 Windows/POSIX 환경에서 즉시 원버튼 검증하기 위한 크로스 플랫폼 대칭형 E2E 통합 테스트 러너(run_e2e_push_test.ps1/sh) 구축(v1.25.0). (24) 프로젝트 헌법 제VI조에 따라 자율 동기화 및 마스터 지식 전파를 통제하는 마스터 AI 가이드 문서인 llms.txt를 프로젝트 4대 핵심 문서 체계로 정식 편입하고 최신화(v1.26.0).
- Added Sections: X. 실 인프라 고가용성 격리 및 TDD 기반 포트 보안 수호
- Deleted Sections: 없음
...
대용량 유입 및 고연산 부하 속에서 전체 서비스의 마비 및 인프라 붕괴를 원천 방지하기 위해 API 요청 응답 흐름과 무거운 백그라운드 처리 과정을 엄격하게 물리적으로 격리합니다. 이미지 리사이징(Pillow) 및 외부 멀티모달 LLM API 호출과 같이 CPU 점유율이 높고 대기 시간이 긴 오프라인 연산은 Celery 비동기 독립 워커 프로세스 내부에서만 실행되어야 하며, API 게이트웨이 서버는 유입 즉시 Redis Broker를 거쳐 임시 대기 상태(Pending, 202) 및 작업 식별자 ID를 즉시 반환하여 프론트엔드의 응답 지연(Latency) 병목을 예방합니다. 또한, 데이터베이스 커넥션 풀(Connection Pool) 크기는 서비스 컨테이너의 가동 사양 및 시스템 자원 용량에 부합하도록 적절히 격리하여 유연하게 최적화 제어합니다.

### III. 3단계 하이브리드 영수증 파싱 전략 및 비용 최적화 (3-Tier Hybrid Pipeline & Cost Optimization)

유료 멀티모달 LLM API 연동에 수반되는 예산 소비를 차단하고 운영 효율을 극대화하기 위해 로컬의 경제성과 상용 LLM의 정밀성을 결합한 3단계 하이브리드 파싱 전략을 구현하고 수호합니다.
1. **1단계 (Local Hybrid)**: PDF(PyMuPDF) 또는 이미지(Tesseract) 로컬 OCR 문자열 1차 획득 후, 로컬 Ollama `qwen2.5:14b-instruct-q4_K_M` 텍스트 모델을 통한 JSON 스키마 구조화 시도 (비용 0원 달성).
2. **2단계 (Cloud Text-only Fallback)**: 로컬 모델의 스키마 붕괴 또는 금액 정합성(Checksum) 검증 실패 시, 이미 확보된 로컬 OCR 문자열만 Gemini-2.5-Flash API로 전송하여 입력 이미지 토큰 비용을 95% 이상 절감하는 초저비용 구조화 시도.
3. **3단계 (Cloud Vision Fallback)**: 로컬 OCR 문자 추출 실패 또는 앞선 텍스트 파싱 오류 발생 시 최후의 보루로 영수증 원본 이미지(WebP 변환 데이터)를 Gemini-2.5-Flash 멀티모달로 송신하여 99% 파싱 무결성 수호.
이 과정에서 기존 정적 정규식 기반 캐시 파이프라인(`BypassParser`, `MerchantTemplate` 캐싱, `promotion.py` 자동 승격, 자가 치유)은 오작동 방지 및 아키텍처 단순화를 위해 전면 비활성화 및 청소 처리합니다.
...
모바일 플랫폼에서의 즉각적이고 안정적인 바로가기 설치(A2HS) 및 네이티브 카메라 연동 최적화 사용자 경험을 웹 표준 사양 위에서 견고하게 실현합니다. 모바일 PWA 접속 시 HTML5 Capture API와 Accept 속성을 바인딩하여 사진첩 리소스를 거칠 필요 없이 스마트폰 네이티브 카메라 셔터를 직접 연동 촬영하도록 제어합니다. 또한 네트워크 전송 대역폭 절감 및 서버의 고용량 이미지 압축 연산 경감을 위해, 업로드 직전 클라이언트 단 HTML5 Canvas API를 가동하여 이미지를 가로 최대 1920px 크기 및 용량 1.5MB 이하로 1차 압축 처리하여 전송합니다. 마지막으로, 서비스 워커의 정상적 등록 및 VAPID 명세의 백그라운드 Web Push 알림 수신을 위한 브라우저 보안 규격을 달성하기 위해 모든 프로덕션 실서버 환경에서 HTTPS SSL 보안 도메인 적용을 강제하되, 로컬 호스트(localhost) 디버깅 환경에 한해서는 브라우저의 보안 컨텍스트 완화 규격에 힘입어 일반 HTTP 가동을 허용하여 개발 편의성을 보장합니다. 단, 서비스 워커(sw.js) 구현 시 크롬 확장 프로그램 등 비-HTTP 요청의 간섭으로 인한 캐시 오류(TypeError: Request scheme is unsupported)를 방지하기 위해, fetch 리스너 시작부에 http 및 https 프로토콜 외의 요청을 사전 차단/우회시키는 필터링 방어 로직을 의무적으로 내장해야 합니다.
...
### X. 실 인프라 고가용성 격리 및 TDD 기반 포트 보안 수호 (Production Isolation & Port Security)

실제 프로덕션(운영) 환경에서는 서비스의 안전한 영속성과 외부 침입으로부터의 원천 차단을 위해 엄격한 컨테이너 리소스 고정 및 네트워크 포트 차단을 수호합니다. CPU 및 메모리 최대 한도(Limits) 및 보장 최소 대역(Reservations)을 명시적으로 고정하여 컨테이너 간의 자원 간섭과 OOM(Out of Memory) 크래시 연쇄 전파를 방어합니다. 또한 데이터베이스 및 Redis 브로커 등 주요 저장소의 물리적 호스트 포트 바인딩 노출을 완전히 차단하고, 오직 prod-bridge 전용 내부 격리 네트워크 대역 내에서만 상호 연결되도록 제어하며, 외부로부터의 진입은 오직 Nginx 리버스 프록시(포트 80 및 443)의 단일 인그레스 통로를 통해서만 허용합니다. 마지막으로, 이와 같은 포트 차단 및 자원 제한 명세의 실제 정상 반영 여부를 배포 파이프라인에서 자동으로 기계적 검증하기 위해 pytest 및 전용 테스트 스위트 기반의 인프라 TDD 사양 검증 체계를 필수적으로 구동 및 보존하도록 선언합니다.
...
* **인공지능 연동 모듈 (AI Core)**: LiteLLM Router 기반 3단계 하이브리드 파이프라인 (로컬 환경: Ollama qwen2.5:14b-instruct-q4_K_M 텍스트 모델 기동 / 프로덕션 환경: Gemini-2.5-Flash. ReceiptLLMClient 및 ReceiptSchema DTO를 통한 구조화 JSON 데이터 규격 강제 바인딩)
...
* **가상 인프라 배포 (Deployment)**: Docker Compose 통합 관리 (api-server, postgres-db, redis-broker, async-worker, nginx-ingress) | 실 운영 환경 CPU/Memory 리소스 한도 고정 및 외부 포트 격리(prod-bridge)
...
* **Phase 2 (비동기 및 고도화) 품질 게이트**:
  ...
  - 3단계 하이브리드 영수증 파싱 파이프라인(Local OCR ➔ Cloud Text-only ➔ Cloud Vision)을 통해, 텍스트 전송 시 이미지 대비 입력 비용을 95% 이상 절감하고 파싱 실패율을 0%에 수렴하도록 E2E 무결성을 보증.
  ...
  - 데스크톱 해상도 넓은 화면에서 대시보드 2열 분할 그리드 레이아웃의 시각적 완성도 및 가독성 검증 완료.
...
**Version: v1.26.0 | Ratified: 2026-05-29 | Last Amended: 2026-06-23
