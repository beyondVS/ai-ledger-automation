<!--
[Sync Impact Report]
- Version Change: v1.19.1 -> v1.20.0
...
1. **1단계 (Local Hybrid)**: PDF(PyMuPDF) 또는 이미지(Tesseract) 로컬 OCR 문자열 1차 획득 후, 로컬 Ollama `gemma4:e4b` 텍스트 모델을 통한 JSON 스키마 구조화 시도 (비용 0원 달성).
...
* **인공지능 연동 모듈 (AI Core)**: LiteLLM Router 기반 3단계 하이브리드 파이프라인 (로컬 환경: Ollama gemma4:e4b 텍스트 모델 기동 / 프로덕션 환경: Gemini-2.5-Flash. ReceiptLLMClient 및 ReceiptSchema DTO를 통한 구조화 JSON 데이터 규격 강제 바인딩)
...
**Version: v1.20.0 | Ratified: 2026-05-29 | Last Amended: 2026-06-17
