<template>
  <div class="top-merchants-card p-6 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl transition-all duration-300 hover:border-slate-700 flex flex-col justify-between h-full">
    <div>
      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-6">이번 달 지출 TOP 3 가맹점</h3>

      <div v-if="merchants && merchants.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
        <template v-for="item in filledMerchants" :key="item.rank">
          <!-- 실제 가맹점 정보 카드 -->
          <div 
            v-if="!item.isEmpty"
            class="merchant-item p-4 bg-slate-800/40 border border-slate-800/80 rounded-xl relative flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 hover:bg-slate-800/60"
          >
            <!-- Rank Badge -->
          ...
          <div 
            v-else
            class="merchant-placeholder p-4 border border-dashed border-slate-800/60 bg-slate-900/40 rounded-xl flex flex-col items-center justify-center text-slate-650 min-h-[120px]"
          >
            <span class="text-xs font-semibold uppercase text-slate-600 font-mono">{{ item.rank }}위</span>
            <p class="text-2xs mt-1.5 text-slate-500 text-center font-medium">분석 대기 중</p>
          </div>
        </template>
      </div>
