  ...
  <div class="auth-container flex items-center justify-center min-h-screen bg-slate-950 px-4 py-8">
    <div class="auth-card w-full max-w-md backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-8 shadow-2xl transition-all duration-300 hover:shadow-purple-500/10 hover:border-purple-500/20">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        <!-- 사용자 아이디 입력 -->
        <div>
          <label for="username" class="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">사용자명 (닉네임)</label>
          <div class="relative">
            <input 
              type="text" 
              id="username" 
      ...
      <div class="mt-8 text-center text-xs text-slate-500">
        아직 계정이 없으신가요? 
        <span @click="navigateToRegister" class="text-purple-400 hover:text-purple-300 font-semibold cursor-pointer underline transition-all ml-1">
          무료로 계정 만들기
        </span>
      </div>
    </div>
