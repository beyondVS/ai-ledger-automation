<template>
  <div class="auth-container flex items-center justify-center min-h-screen bg-slate-950 px-4 py-8">
    <div class="auth-card w-full max-w-md backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-8 shadow-2xl transition-all duration-300 hover:shadow-purple-500/10 hover:border-purple-500/20">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-5">
        <!-- 이름(닉네임) 입력 -->
        <div>
          <label for="username" class="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">이름 (닉네임) *</label>
          <div class="relative">
            <input 
              type="text" 
              id="username" 
              v-model="username" 
              placeholder="홍길동"
              class="w-full bg-slate-900/60 border border-slate-800 rounded-xl px-4 py-3 text-slate-200 text-sm placeholder-slate-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/30 transition-all"
            />
          </div>
        </div>
        ...
        <!-- 이메일 입력 -->
        <div>
          <label for="email" class="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">이메일 주소</label>
          <div class="relative">
            <input 
              type="email" 
              id="email" 
        ...
        <!-- 비밀번호 입력 -->
        <div>
          <label for="password" class="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">비밀번호 *</label>
          <div class="relative">
            <input 
              type="password" 
              id="password" 
        ...
        <!-- 비밀번호 확인 입력 -->
        <div>
          <label for="passwordConfirm" class="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">비밀번호 확인 *</label>
          <div class="relative">
            <input 
              type="password" 
              id="passwordConfirm" 
      ...
      <div class="mt-8 text-center text-xs text-slate-500">
        이미 계정이 있으신가요? 
        <span @click="navigateToLogin" class="text-purple-400 hover:text-purple-300 font-semibold cursor-pointer underline transition-all ml-1">
          로그인하러 가기
        </span>
      </div>
    </div>
