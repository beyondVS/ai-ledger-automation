...
def test_duplicate_ledger_block_and_failed_task_routing():
    """
    ...
    """
    user = User.objects.create(email="merchant_dup@example.com")
    ...
    # 1. 1차 인서트 시도: 성공해야 함
    result1 = create_ledger_transactional(user_id_str, ledger_data, items_data)
    assert result1["status"] == "SUCCESS"
    assert Ledger.objects.filter(vendor_registration_number="1208147528").count() == 1
    
    # 2. 2차 인서트 시도: 동일 페이로드 적재 시 DB 고유 키 위배 발생
    with pytest.raises(IntegrityError):
        create_ledger_transactional(user_id_str, ledger_data, items_data)
        
    # 3. [UNIQUE 차단 수호]: 데이터베이스에 2번째 영수증 행이 적재되지 않았음을 증명 (개수 여전히 1개)
    assert Ledger.objects.filter(user=user, vendor_registration_number="1208147528").count() == 1
    
    # 4. [FailedTask DLQ 격리 수집 검증]: 2번째 인서트 실패 페이로드가 FailedTask에 영구 격리 적재되었는지 확인
    assert FailedTask.objects.filter(user=user, task_type="API_LEDGER_INGEST_DUPLICATE").exists()
    
    failed_log = FailedTask.objects.filter(user=user, task_type="API_LEDGER_INGEST_DUPLICATE").first()
    assert failed_log.error_message is not None
    
    # 페이로드 역직렬화 및 복원력 검증
    payload = json.loads(failed_log.raw_payload)
    assert payload["ledger_data"]["vendor_name"] == "홍콩반점"
    assert float(payload["ledger_data"]["total_amount"]) == 24000.00
