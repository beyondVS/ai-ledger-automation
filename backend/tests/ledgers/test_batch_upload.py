...
class CeleryTaskRetryAndFailureTests(TransactionTestCase):
    ...
    def test_celery_task_retries_on_transient_failure_and_finally_fails(self):
        ...
        with patch(
            "utils.llm_client.ReceiptLLMClient.parse_receipt_cloud_vision",
            side_effect=ValueError("Gemini API Connection Refused"),
        ):
        ...
        with patch(
            "utils.llm_client.ReceiptLLMClient.parse_receipt_cloud_vision", side_effect=ValueError("Gemini API Down")
        ):
