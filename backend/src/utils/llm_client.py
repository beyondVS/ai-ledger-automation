...
COMMON_RECEIPT_PROMPT = (
    ...
    "   - **중요 - 상품명 보존 규칙**:\n"
    "     - 상품명 우측에 붙은 수치 정보나 기호(예: `×6,480`, `(100g)`, `*6` 등)가 단순 구매 수량을 나타내는 단순 수량 표기가 아니며, 상품 자체의 옵션명, 패키지 규격(예: 게임 재화 패키지 등)인 경우에는 **절대로 잘라내거나 변형하지 말고 원본 텍스트 전체를 상품명(item_name)으로 보존**하십시오.\n"
    "     - 특히 상품명 내에 쉼표(`,`)나 곱하기 기호(`×`, `*`)가 들어간 상품 패키지 스펙(예: `모노크롬×6,480`)을 단지 가격의 천단위 구분기호로 오해하여 일부 숫자를 유실시키지 마십시오. 패키지 상품(예: `모노크롬×6,480` 패키지 1개 구입)의 경우, 수량(quantity)은 `1`로 처리하되 상품명(item_name)은 원형인 `모노크롬×6,480` 전체를 그대로 유지해야 합니다."
)
