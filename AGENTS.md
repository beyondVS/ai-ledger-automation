...
- **양대 쉘(PowerShell & Bash) 대칭적 동등 지원**: 로컬 기동 스크립트나 인프라 관리 도구를 생성 또는 수정할 때, 에이전트는 반드시 Windows(PowerShell, `*.ps1`)와 macOS/Linux/WSL(Bash, `*.sh`) 양대 실행 환경 모두에 호환되는 완전한 대칭형 스크립트를 제공해야 합니다. 한쪽 플랫폼으로 치우친 툴링 설계는 엄격히 금지됩니다.
- **3대 코어 문서(README, AGENTS, Constitution) 자율 교차 동기화**: 로컬 셋업 지침, 기술 스택, 아키텍처적 변동 사항이 발생할 시, 에이전트는 사용자의 별도 지시가 없더라도 주도적으로 `README.md`, `AGENTS.md`, `.specify/memory/constitution.md` 3대 핵심 문서 간에 누락이나 불일치가 발생하지 않도록 상호 연관된 변경 내용을 유기적으로 교차 검증하고 자동으로 동기화 업데이트를 완수해야 합니다.
