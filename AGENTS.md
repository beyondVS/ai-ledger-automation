...
- **Language / Framework**: `Python 3.13 (Django REST Framework, google-genai SDK 및 litellm.Router 다이내믹 라우팅)` & `Vue.js 3 (PWA, Tailwind CSS)`
...
- **비직관적 비즈니스 로직**:
  - 자가 제안(Auto-Generation)되는 사업자번호 기반 정규식 캐싱 규칙은 무조건 `is_verified: false`로 최초 적재되나, 해당 템플릿의 정규식 규칙이 LLM 분석 및 실제 데이터 대조 과정에서 동일한 패턴으로 연속 3회 완벽하게 일치하면 `is_verified: true`로 자동 승격(Auto-Promotion)되어 바이패스 엔진에 즉시 반영됨.
  - 바이패스 파싱 도중 에러가 발생하거나 사용자가 가계부 수동 수정을 통해 데이터를 정정하면, 해당 가맹점 템플릿은 즉시 '미검증' 상태로 강등되며 백그라운드 Celery 태스크(`self_heal_template_task`)를 통해 Gemini API를 호출하여 정규식을 자동 갱신(자가 치유)함.
  - 특정 가맹점 템플릿의 자가 치유 시도가 3회 연속 실패할 경우, 해당 템플릿은 완전히 블랙리스트(`is_blacklisted: true`)로 격리되어 바이패스 매칭 후보에서 영구 배제됨.
  ...
  - 어드민 전용 전체 템플릿 관리 대시보드(통계, 페이지네이션 리스트, 상세조회) 및 일반 사용자 전용 내 가맹점 템플릿 관리(/my/templates) 기능 (※ 4주 로드맵에서 제외되어 차후 확장 계획 백로그로 이관됨. 현재 단계에서는 구현하지 않음).
