...
\[AI Analysis Layer\] \--\> (7) 멀티모달 LLM API 호출 (google-genai SDK를 활용한 Gemini-2.5-Flash 호출 혹은 로컬 개발 대역 LiteLLM 라우터를 경유한 Ollama gemma4:e4b 분석 연동)  
                                │  
                                ▼  
...
* [x] **14일차:** 2주차 동기식 MVP 완전체 통합 테스트 및 AI 분석 모듈 현대화. 지원 종료된 `google-generativeai` SDK를 2026년 기준 최신 `google-genai` SDK로 전격 마이그레이션하고, 로컬 디버깅 환경 대응을 위해 `litellm` 라우터와 Ollama `gemma4:e4b` 연동 가교 구축 완료. PDF 파일 업로드 시 Pillow 전처리 오류를 예외 분기 처리하여 원본 PDF Bytes 데이터 및 MIME 타입을 Gemini API에 직접 전달하는 전처리 고도화 구현. 백엔드 pytest 49개 및 프론트엔드 테스트 38개 전원 그린 패스 완료.

### **6.3. 3주차: 비동기 분산 아키텍처 및 비용/보안 고도화 (15일차 \~ 21일차)**

* [ ] **15일차:** 비동기 처리를 위한 Redis 인메모리 스토어 인프라 구축 및 Docker Compose 환경 통합. 2주차까지 설계된 동기식 가동 서버를 Django 메인 서버와 Celery 백그라운드 워커 서버로 역할 분리 설계.  
...
* [ ] **22일차:** PWA Manifest/Service Worker 캐시 연동 및 HTML5 Capture API 모바일 기기 카메라 연동.  
* [ ] **23일차:** 스마트 단말기(iOS, Android)의 beforeinstallprompt 제어 배너 설계 및 사파리 수동 툴팁 A2HS 레이아웃 가이드 구현.  
* [ ] **24일차:** 백그라운드 VAPID 표준 V2 VAPID 푸시 발송 큐(Notification Queue) 파이프라인 개설 및 FCM/APNs 연동 웹 푸시 비동기 발송 모듈 독립 기동.  
* [ ] **25일차:** 비동기 워커 알림 소비 태스크 오프라인 수신 단말 E2E 모바일 푸시 알림 도달 및 디바이스 캐싱 데이터 무결 테스트 완료.  
* [ ] **26일차:** 대규모 실 인프라용 docker-compose.prod.yml 튜닝 설계 및 호스트 포트 접근 보안 제어.  
* [ ] **27일차:** 서버 실 프로덕션 환경 HTTPS SSL 보안 인증 체제 완수 및 Nginx 리버스 프록시 SSL HTTPS 통신 레이어 탑재.  
* [ ] **28일차:** 가맹점 정적 캐시 템플릿 검증 어드민 수동 승인 필터 가동 및 전 과정 E2E 인바운드 통합 알림망 정식 릴리즈 완료.