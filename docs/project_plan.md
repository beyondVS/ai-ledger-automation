...
* [ ] **18일차:** 비용 통제 엔진 핵심(뼈대) 구현. `MerchantTemplate` 모델에 검증 여부(`is_verified`) 컬럼 추가 설계. 10자리 사업자등록번호를 추출하여 가맹점을 식별하고, `is_verified: true` 상태인 경우에만 LLM 호출을 우회(Bypass)하여 파싱하며 실패 시 LLM으로 폴백(Fallback)하는 기본 안전장치 구축. 신규 가맹점은 LLM 파싱 성공 후 정규식 규칙을 산출해 `is_verified: false` 상태로 자동 제안(자가 학습)하는 등록 파이프라인 구현.  
...
* [ ] **20일차:** 외부 이메일 인바운드 수신용 웹훅(SendGrid Inbound Parse 등) 연동 라우터 개설 및 첨부파일 PDF를 수신하여 Celery 비동기 작업 큐로 즉시 넘기는 데이터 가교 모듈 작성.  
* [ ] **21일차:** 이메일 전초 기지 보안 구현. 발신자 위변조 방지를 위한 SPF/DKIM 정합성 체크 유틸리티 구축 및 가입 사용자당 가계부에 메일을 보낼 수 있는 발신용 이메일 화이트리스트(최대 3개 매핑) 테이블 검증 적용.  

### **6.4. 4주차: PWA 플랫폼 최적화, Web Push 및 프로덕션 배포 (22일차 \~ 28일차)**

* [ ] **22일차:** 3주차 비동기 아키텍처 튜닝 및 부하 테스트. 영수증 50종을 일시에 웹과 메일로 동시에 유입시킨 뒤, API 서버 마비 없이 Redis 대기열을 거쳐 Celery 백그라운드 워커가 순차적으로 완벽히 데이터 적재 트랜잭션을 실행해 나가는지 성능 및 정합성 검증 완료.  
...
* [ ] **28일차:** 서버 실 프로덕션 환경 HTTPS SSL 보안 인증 및 Nginx 리버스 프록시 탑재, 전 과정 E2E 인바운드 통합 알림망 정식 릴리즈 완료.