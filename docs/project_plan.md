...
* [ ] **11일차:** **[추가 계획] 프론트엔드 로그인 상태 체크(인증 토큰 기반 라우터 가드 구현) 및 실제 사용자 로그인/회원가입 UI 화면 개발.** 모바일 사용자 전용 클라이언트 사이드 이미지 리사이징 모듈 내장 (HTML5 Canvas API를 이용하여 업로드 단추를 누르기 직전 1000px 규격 최적화 압축 처리 가동하여 네트워크 트래픽 절감).  
